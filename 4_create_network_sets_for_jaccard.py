...
#The alphabetical order will preserve unicity of the edges
def alph_order(a: str, b: str) -> str:
    return a.lower() + '_' + b.lower() if a.lower() < b.lower() else b.lower() + '_' + a.lower()

#Prepare the block unique identifiers time stamp
with open('sample_data/sample_block_timestamp.csv') as f:
    reader = csv.reader(f)
    for line in reader:
...
for filename in filenames:
    #We consider only tokens with at least 10000 transactions
    if count_lines(filename)>=10000:
        new_nodes=set()
        new_edges=set()
        total_number+=1
        print("Analysing token in {}".format(filename))
